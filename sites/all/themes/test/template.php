...
// Add Zen Tabs styles
if (theme_get_setting('test_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'test') . '/css/tabs.css');
}
...
function test_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function test_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('test_breadcrumb');
  if ($show_breadcrumb == 'yes' || ($show_breadcrumb == 'admin' && arg(0) == 'admin')) {



    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('test_breadcrumb_home');
    if (!$show_breadcrumb_home) {
      array_shift($breadcrumb);
    }
    // Return the breadcrumb with separators.
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('test_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('test_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('test_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function test_add_conditional_styles() {
...
function test_menu_link(array $variables) {
 ...
 */
function test_preprocess_menu_local_task(&$variables) {
...
function test_menu_local_tasks(&$variables) {
  $output = '';
